...
function Prim(graph) {
    ...
    while(min[1] != null || checknodes.length !=0) {
        if(edge_exists(min)==0){  
            ...
            node = checknodes[Math.round(Math.random()*(checknodes.length-1))];
            result.push(node);
            removefrom_checknodes(node);
            usedNodes[node] = true;
            min = findMin(graph);
...
function printedges(){
    var totwt = 0;
    for(var i in resultedges){
        //console.log(array[i][2]+" - "+array[i][1]+"   "+array[i][0]);
        totwt=totwt+resultedges[i][0];
    }
...
fs.readFile('data/fb_nodes.txt', function(err, data) {
    ...
    fs.readFile('data/fb_edges.txt', function(err, data) {
        ...
        /*var result = Prim(g);
        printedges(result);*/
        var p = new Parallel([g,g,g,g]);
        function add(d) { return d[0].concat(d[1]); }
        p.map(Prim).then(printedges);
