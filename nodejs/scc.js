...
fs.readFile('data/fb_nodes.txt', function(err, data) {
  ...
  fs.readFile('data/fb_edges.txt', function(err, data) {
    ...
    for(i in array) {;
      var fst = parseInt(array[i].split(" ")[0]);
      var snd = parseInt(array[i].split(" ")[1]);
      adj_list[fst].push(snd);
      //adj_list[snd].push(fst);
    }

      //g.edgearray=ksort(g.edgearray, function(a, b){ return a.capacity < b.capacity;});
       // console.log(adj_list);
      console.log(stronglyConnectedComponents(adj_list));
  });
});
